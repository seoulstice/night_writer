...
class NightWriter
  ...
  def split_input_into_array
      ...
      @split_input = @split_input.map {|a| a.scan(/.{2}/)}
      @split_input.each_with_index do |value, index|
        line_1_array << value if index == 0
        line_2_array << value if index == 1
        line_3_array << value if index == 2
      end
    end

   def translate_each_letter
      line_1_array.flatten.each_with_index do |value, index|
        line_1 << value << line_2_array.flatten[index] << line_3_array.flatten[index]
      end
      binding.pry
    end
